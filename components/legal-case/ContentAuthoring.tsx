    ...
    <div className="space-y-6">
      <div className="px-6 py-4 space-y-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Select an outage, choose a template, generate and submit communications.</p>
        </div>
            ...
            <div key={step.number} className="flex items-center">
              <div
                className={`flex items-center justify-center w-8 h-8 rounded-full border-2 text-sm font-medium cursor-pointer ${
                  step.active ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 text-gray-500"
                }`}
              >
                {step.number}
              </div>
              <span
                className={`ml-2 text-sm font-medium cursor-pointer ${step.active ? "text-gray-900" : "text-gray-500"}`}
              >
                {step.title}
              </span>
            </div>
      ...
      <div className="px-6 grid grid-cols-4 gap-6">
        ...
        <div className="col-span-1">
          <Card className="border border-gray-200">
            <CardContent>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Select Outage</h3>
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <div className="flex items-center space-x-3">
                        <span className="font-medium text-gray-900">{outage.id}</span>
                        <span className="text-sm text-gray-500">{outage.stage}</span>
                      </div>
                      <span className="text-sm text-gray-600">{outage.customers}</span>
                    </div>
                    <div className="text-sm text-gray-600 mb-1">{outage.location}</div>
                    <div className="flex justify-between items-center">
                      <Badge
                        variant={outage.type.includes("Major") ? "destructive" : "secondary"}
                        className="cursor-pointer"
                      >
                        {outage.type}
                      </Badge>
                      <div className="text-xs text-gray-500">Identified: {outage.identified}</div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-600 bg-gray-50 p-3 rounded whitespace-pre-line">
                    {(() => {
                      if (!selectedTemplate || !getCurrentTemplate() || !getCurrentOutage()) {
                      ...
                      return (
                        <>
                          ...
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            <span className="text-xs text-gray-500">• Preview populated with selected outage data</span>
                          </div>
                          <div className="mt-1">
                            <span className="text-xs text-green-600 font-medium">
                              Ready to generate: Template and outage data are selected.
                            </span>
                          </div>
                        </>
                      )
        ...
        <div className="col-span-1">
          <Card className="border border-gray-200">
            <CardContent>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-gray-900">Generated Communication Draft</h3>
                {generatedDraft && (
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs text-gray-500">
                      {getWordCount(generatedDraft).words} words • {getWordCount(generatedDraft).characters} characters
                    </span>
                  </div>
                )}
              </div>
              ...
              {generatedDraft ? (
                <div className="space-y-4">
                  <div className="text-sm text-gray-600 mb-2">{getGeneratedDescription()}</div>
                  <Textarea
                ...
                <p className="text-sm text-gray-500 text-center py-8">No draft generated</p>
              )}
            </CardContent>
          </Card>
        </div>
